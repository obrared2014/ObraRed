...
function traeInformePresupuestosCasa($idPersona){
    //$nombre=$_SESSION['nombre'].' '.$_SESSION['ap_paterno'];
  $link = @mysql_connect("localhost", "root","root")
      or die ("Error al conectar a la base de datos.");
  @mysql_select_db("db_obrared", $link)
...
//	  "WHERE nombre='".$nombre."'"." and precioTotal".$tipo.">'0' and id_casa='0'".
//	  "order by precioTotal".$tipo."";
  $query="call mostrarMisPresupuestosCasa(".$idPersona.")";
  $result = mysql_query($query);
  $numero = 0;
  while($row = mysql_fetch_array($result))
  {
    ...
    echo "<td width=\"10%\" style=\"text-align: center;\"><font face=\"verdana\">" . 
	    $row["fecha"] . "</font></td>";
    echo "<td width=\"20%\"><font face=\"verdana\">Cotizaci&oacute;n para una Casa de ".$row["metrosCuadrados"]." m2</font></td>";
    echo "<td  class='text-right' width=\"10%\"><font face=\"verdana\">" . 
	    '$'. number_format($row['total'],0,',','.')."</font></td>";    
    echo "<td width=\"15%\" align=\"center\"><font face=\"verdana\"><a href=\"javascript:mostrarPresupuestoCasa(".$row["idCasa"].",'Casa');\"> 
	    <button type='button' class='btn btn-info'><span class='glyphicon glyphicon-search'></span></button></a></font></td></tr>";    
...
function mostrarPresupuestoCasa($idCasa){
//        $idPresupuesto=474;
        $link = @mysql_connect("localhost", "root","root")
            or die ("Error al conectar a la base de datos.");
        @mysql_select_db("db_obrared", $link)
...
//            session_start();
            $_SESSION['que']                        = 'Casa';
            $_SESSION['m2m3']                       = '2';
            $_SESSION['metrosCasa']                 = $row['ancho']*$row['largo'];
